...
import { getGeneratedCv } from "@/lib/applications";
 ...
 *
 * Sta dentro `<Suspense>` nel dettaglio dell'annuncio perché fa quattro letture
 * — candidatura, task, battito, URL firmato — e l'ultima è una chiamata di rete
 * verso Supabase. Il dettaglio dell'annuncio è la parte per cui si è aperta la
 * pagina: non deve aspettare un URL firmato per comparire.
 */
export async function CvSection({ matchId }: { matchId: number }) {
  const [cv, task, worker] = await Promise.all([
